...
async function registerServiceWorker(): Promise<ServiceWorkerRegistration> {
  if (!('serviceWorker' in navigator)) {
    throw new Error('Service Workers ne sont pas supportés dans ce navigateur');
  }
...
export async function sendNewAudioNotification(audioTitle: string, audioUrl: string) {
  try {
